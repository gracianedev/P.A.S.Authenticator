...
// Método para verificar se o OTP ainda é válido
    public boolean isValidTime(Instant timestamp) {
        Instant expirationTime = timestamp.plusSeconds(EXPIRATION_TIME_IN_MINUTES * 60);
        return Instant.now().isBefore(expirationTime);
    }

    public boolean isValidOtpUser(User user, String codeInput) {
        List<Otp> otps = user.getOtpList();
        if (otps == null || otps.isEmpty()) {
            return false;
        }

        String lastOtp = (otps.get(otps.size() - 1)).getCode();
        return  lastOtp.equals(codeInput) ;
    }
